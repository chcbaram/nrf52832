...
typedef struct
{
  ...
  swtimer_mode_t  Timer_Mode;    // 타이머 모드
...
static swtimer_t  swtimer_tbl[SWTIMER_MAX_CH];           // 타이머 배열 선언
...
void swtimerISR(void)
{
  ...
  for (i=0; i<SWTIMER_MAX_CH && i<sw_timer_handle_index; i++)     // 타이머 갯수만큼
  {
...
{
  swtimer_tbl[TmrNum].Timer_Mode = mode;       // 모드설정
  swtimer_tbl[TmrNum].TmrFnct    = Fnct;       // 실행할 함수
  swtimer_tbl[TmrNum].TmrFnctArg = arg;        // 매개변수
  swtimer_tbl[TmrNum].Timer_Ctn  = time_ms;
  swtimer_tbl[TmrNum].Timer_Init = time_ms;
}

void swtimerStart(uint8_t TmrNum)
{
  if(TmrNum < SWTIMER_MAX_CH)
  {
