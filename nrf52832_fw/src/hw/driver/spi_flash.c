...
bool spiFlashInit(void)
{
  ...
  {
    if (id_tbl[1] == 0xEF && id_tbl[2] == 0x40 && id_tbl[3] == 0x17)
    {
...
bool spiFlashReset(void)
{
  ...
  p_spi->PSEL.SCK  = NRF_GPIO_PIN_MAP(0, 20);
  p_spi->PSEL.MISO = NRF_GPIO_PIN_MAP(0, 23);
  p_spi->PSEL.MOSI = NRF_GPIO_PIN_MAP(0, 22);
  p_spi->FREQUENCY = SPIM_FREQUENCY_FREQUENCY_M8;
  p_spi->CONFIG    = (SPIM_CONFIG_CPOL_ActiveHigh << SPIM_CONFIG_CPOL_Pos) |
                     (SPIM_CONFIG_CPHA_Leading    << SPIM_CONFIG_CPHA_Pos) |
                     (SPIM_CONFIG_ORDER_MsbFirst  << SPIM_CONFIG_ORDER_Pos);


  p_spi->ENABLE = SPIM_ENABLE_ENABLE_Enabled;
...
bool spiFlashTransfer(uint8_t *tx_buf, uint8_t *rx_buf, uint32_t length, uint32_t timeout)
{
  ...
  if (rx_buf == NULL)
  {
    p_spi->TXD.PTR    = (uint32_t)tx_buf;
    p_spi->TXD.MAXCNT = length;
    p_spi->RXD.PTR    = (uint32_t)NULL;
    p_spi->RXD.MAXCNT = 0;
  }
  else if (tx_buf == NULL)
  {
    p_spi->TXD.PTR    = (uint32_t)NULL;
    p_spi->TXD.MAXCNT = 0;
    p_spi->RXD.PTR    = (uint32_t)rx_buf;
    p_spi->RXD.MAXCNT = length;
  }
  else
  {
    p_spi->TXD.PTR    = (uint32_t)tx_buf;
